...
export class ProgramWindow {
  ...
  resize(size) {
    ...
    const prune = (length, position, screenSize) => {
      length -= (position + length) > screenSize ? (position + length) - screenSize : 0;
      return length;
    }
  ...
  move(position) {
    ...
    const prune = (coord, size, screenSize) => {
      coord -= (coord + size) > screenSize ? (coord + size) - screenSize : 0;
      return coord;
    }
