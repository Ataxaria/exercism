...
export class TranslationService {
  ...
  free(text) {
    return this.api.fetch(text)
      .then((text) => text.translation);

    // Equivalent try/catch
    /*
    try {
      const promise = await this.api.fetch(text);
  ...
  premium(text, minimumQuality) {
    ...
    return this.api.fetch(text)
      ...
      .catch((error) => {
        ...
        return this.request(text).then(() => this.api.fetch(text).then(qualityCheck));
    });
  }
}
